...
while i < len(travel_costs):
    ...
    while j < len(travel_costs[i]):
        ...
        if travel_costs[i][j] > max_expense:
            max_expense = travel_costs[i][j]
        j += 1
    max_costs.append(max_expense)
    print(f"The maximum expense for trip {i+1}: {max_expense}")
    print(f"The total cost cost for the trip {i+1} is {total_cost}")
    i += 1
# Testing
